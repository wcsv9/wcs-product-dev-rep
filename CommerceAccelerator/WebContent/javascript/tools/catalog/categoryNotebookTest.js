 ...
 /*
  ...
  function validGeneral()
  {
     ...
     if (general != null )
     {
        ...
        var code = general.code;
		var longdesc = general.longdescription;
		var keywrd = general.keyword;
		
	if ( !isValidUTF8length(code, 254)  )
      	{
     		gotoPanel("CategoryGeneral", "fieldSizeExceeded_code");
   	  	return false;
      	}

     	
     	if ((code == "") || ( code == null )) {
     	    gotoPanel("CategoryGeneral", "misscode_code");
            return false;
     	}
	...
	if ( !isValidUTF8length(name, 254)  )
        ...
        if ( !isValidUTF8length(longdesc, 4000)  )
      	{
     	     gotoPanel("CategoryGeneral", "fieldSizeExceeded_longdescription");
   	     return false;
      	}

        if ( !isValidUTF8length(keywrd, 254)  )
      	{
     	     gotoPanel("CategoryGeneral", "fieldSizeExceeded_keyword");
   	     return false;
      	}
  ...
  function validParent () {
   ...
   if ( category_parent != null ) {
	  ...
	  deletedCategories = top.get( "deletedCategory", null );
          ...
          deletedCategories[ newIndex ] = thisCategoryId;
