...
function userSavePanelData() {
	...
	if (model) {
		var mod = model;		
		top.mccbanner.trail[1].contractSearchParm = mod;
	}
...
function userLoadPanelData () {
	...
	var previousModel = top.mccbanner.trail[1].contractSearchParm;
	if (previousModel) {
		//Check to see the search are coming from the right search page by comparing page type.
		if (top.mccbanner.trail[1].contractSearchParm.pageType == CONTENTS.document.searchForm.pageType.value) {
			model = previousModel;
		}
	}
	//alert (dumpObject (model));
