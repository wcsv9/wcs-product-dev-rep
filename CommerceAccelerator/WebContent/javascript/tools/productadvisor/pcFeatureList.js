...
function updateLinkToPage(Index,vecPos)
{
	var remVector = top.get("vectorDataRemove");
...
//test
	...
	var tableId = 'pcFeatureListSummaryID';
	var table = getTable(tableId);

	var rVector = top.get("vectorDataRemove");

	var sortArray = top.get("sortarrayData");
	var prodLinkList = top.get("prodLinkListData");
	...
	for(var i=1;i<tlen;i++){
		...
		insDropDownList(tableId,i,4,obj.name,"updateLinkToPage(selectedIndex,"+(i-1)+")",prodLinkList,isLink);
	...
	var checkLength = document.pcForm.elements.length;
	for (var i=0; i<checkLength; i++) {
	    var e = document.pcForm.elements[i];
	    if (e.type == 'checkbox') {
     		if (e.name != 'select_deselect' && !e.checked && e.name == (shifted_index-1).toString()) {
			document.pcForm.elements[i].click();
     		}
    	   }
	}
	return;
