...
*-------------------------------------------------------------------*/
    ...
    function submitFinishHandler(submitFinishMessage)
    {
   	// Get the attachment Id.
    	...
    	var replaceAttachmentIdentity = top.getData("replaceAttachmentIdentity",1);
        if (replaceAttachmentIdentity != undefined || replaceAttachmentIdentity != null)
        {
            for (var i = 0;i < allAttachments.length;i++)
            {
            	if (allAttachments[i].identity == replaceAttachmentIdentity)
            	{
                    allAttachments[i].attachMarkForDelete = "1";
            	}
            }
        }
