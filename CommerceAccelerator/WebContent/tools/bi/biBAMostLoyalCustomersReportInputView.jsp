   ...
   String result =        "<FORM NAME=" + container + ">\n" +
                               ...
                               "        <TR>\n" +
                                              ...
                                              "            <INPUT TYPE=RADIO NAME=sortBy VALUE=All>\n" +
                                                        "                     <label for= s1>" +
                                              "               " + biNLS.get("totalOrderValue") + "\n </label>" +
                                              "                      </INPUT>\n" +
                                              "            <BR>\n" +
                            "            <BR>\n" +
                                              "            <INPUT TYPE=RADIO NAME=sortBy VALUE=PayA>\n" +
                                                        "                     <label for= s2>" +
                                              "               " + biNLS.get("numberOfOrders") + "\n </label>" +
                                              "                      </INPUT>\n" +
                                              "            <BR>\n" +       
                                                 "            <BR>\n" +       
                                              "         </TD>\n" +
...
<link rel=stylesheet href="<%= UIUtil.getCSSFile(biLocale) %>" type="text/css">
   ...
   <title><LABEL><%=biNLS.get("biBAMostLoyalCustomersReport")%></LABEL></title>
...
<SCRIPT>
   ...
   function returnSortByOptionOrderValue(container) {
      return document.forms[container].sortBy[0].checked;
   }
   function returnSortByOptionNumOfOrders(container) {
 ...
 ///////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   <script>
      ...
      function initializeValues()
      {
		 ...
		 onLoadSortOrderByOption("myHelperBAMostLoyalCustomers");
		 ResetValues();
      ...
      function savePanelData()
      {
         ...
         setReportFrameworkParameter("XMLFile","bi.biBAMostLoyalCustomersReportOutputDialog");
         setReportFrameworkReportXML("bi.biBAMostLoyalCustomersReport");
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////
         // Specify the report framework particulars
         ////////////////////////////////////////////////////////////////////////////////////////////////////
         if(returnSortByOptionOrderValue("myHelperBAMostLoyalCustomers")){

			// ASC & DESC CHECKING QUERY
				if(returnOrderbyDesc("myHelperBAMostLoyalCustomers")){
					setReportFrameworkReportName("biBAMostLoyalCustomersReport"+ document.all.time[document.all.time.selectedIndex].value+"OrderValue");
				}
				else{
					setReportFrameworkReportName("biBAMostLoyalCustomersReport"+ document.all.time[document.all.time.selectedIndex].value+"OrderValueAscend");
				}
		 }
		  else{
				if(returnOrderbyDesc("myHelperBAMostLoyalCustomers")){
					setReportFrameworkReportName("biBAMostLoyalCustomersReport"+ document.all.time[document.all.time.selectedIndex].value+"NumOfOrders");
				}
				else{
					setReportFrameworkReportName("biBAMostLoyalCustomersReport"+ document.all.time[document.all.time.selectedIndex].value+"NumOfOrdersAscend");
				}
       	  }
		...
		else
		{
         ...
         if(returnSortByOptionOrderValue("myHelperBAMostLoyalCustomers")){
			...
			// ASC & DESC CHECKING QUERY
				if(returnOrderbyDesc("myHelperBAMostLoyalCustomers")){
					setReportFrameworkReportName("biBAMostLoyalCustomersReportOrderValue");
				}
				else{
					setReportFrameworkReportName("biBAMostLoyalCustomersReportOrderValueAscend");
				}
         }
		 else{
				if(returnOrderbyDesc("myHelperBAMostLoyalCustomers")){
					setReportFrameworkReportName("biBAMostLoyalCustomersReportNumOfOrders");
				}
				else{
					setReportFrameworkReportName("biBAMostLoyalCustomersReportNumOfOrdersAscend");
				}
       	  }
...
<body onload="initializeValues()" class="content">
	...
	<h1><LABEL><%=biNLS.get("biBAMostLoyalCustomers")%></LABEL></h1>
	<LABEL><%=biNLS.get("biBAMostLoyalCustomersDescription") %></LABEL>
	<p></p>
   <LABEL><%=biNLS.get("biBAMostLoyalCustomersReportSelection")%></LABEL>
   <p>
...
<table border="0" bordercolor="black" CELLPADDING="0" CELLSPACING="0">
	...
	<div id=pageBody style="display: block; margin-left:0">
		<%=generateSortOrderByOption("myHelperBAMostLoyalCustomers", biNLS)%>
	</div>
