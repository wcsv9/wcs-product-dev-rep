 ...
 <link rel=stylesheet href="<%= UIUtil.getCSSFile(biLocale) %>" type="text/css">
    ...
    <title><LABEL><%=biNLS.get("eMailActivityEvents")%></LABEL></title>
    ...
    <SCRIPT SRC="/wcs/javascript/tools/reporting/ReportHelpers.js"></SCRIPT>
     ...
     <SCRIPT>
        ...
        function initializeValues()
       {
 		 ...
 		 ResetValues();
 		 onLoadStatOption("myHelperEMailActivityEvents");
       ...
       {
 		 saveStatOption("myHelperEMailActivityEvents");
          setReportFrameworkOutputView("DialogView");
 		 setReportFrameworkParameter("XMLFile","bi.biEMailActivityEventsReportOutputDialog");
          setReportFrameworkReportXML("bi.biEMailActivityEventsReport");
 
 
 	  	if (document.forms["myHelperEMailActivityEvents"].rbsort[0].checked) {
 			 setReportFrameworkParameter("sortOrder","DESC");
 	  	} else {
 			 setReportFrameworkParameter("sortOrder","ASC");
 	  	}
 
 		if (document.forms["myHelperEMailActivityEvents"].rbname[0].checked) {
 			   setReportFrameworkParameter("sortBy","1");
 		} else if (document.forms["myHelperEMailActivityEvents"].rbname[1].checked) {
 			   setReportFrameworkParameter("sortBy","2");
 		} else if (document.forms["myHelperEMailActivityEvents"].rbname[2].checked) {
 			   setReportFrameworkParameter("sortBy","3");
 		} else if (document.forms["myHelperEMailActivityEvents"].rbname[3].checked) {
		 			   setReportFrameworkParameter("sortBy","4");
 		} else if (document.forms["myHelperEMailActivityEvents"].rbname[4].checked) {
		 			   setReportFrameworkParameter("sortBy","5");
 		} else if (document.forms["myHelperEMailActivityEvents"].rbname[5].checked) {
		 			   setReportFrameworkParameter("sortBy","6");
 		}
 		 else if (document.forms["myHelperEMailActivityEvents"].rbname[6].checked) {
				          setReportFrameworkParameter("sortBy","7");
		 		}
 		
 		
 		
 		setReportFrameworkReportName("biEMailActivityEventsReport");
  ...
  function onLoadStatOption(container)
    ...
    function returnStatOptionClickedAbandoned(container) {
 ...
 <BODY ONLOAD="initializeValues()" CLASS=content>
    ...
    <h1><LABEL><%=biNLS.get("eMailActivityEvents") %></LABEL></h1>
    <LABEL><%=biNLS.get("eMailActivityEventsDescription") %></LABEL>
    <p>
 ...
 <table border="0" bordercolor="black" CELLPADDING="0" CELLSPACING="0">
 	...
 	<DIV ID=pageBody STYLE="display: block; margin-left:0">
 		<FORM NAME=myHelperEMailActivityEvents>
 					 ...
 					 <TR>
 						 <TD ALIGN=left VALIGN=TOP>
 							<TABLE border=0 bordercolor=black CELLPADDING=0 CELLSPACING=0 width=200>
 							 <TR HEIGHT=5>
 							   <TD ALIGN=left VALIGN=TOP>
 							<INPUT TYPE=RADIO NAME=rbname id="s0" VALUE=All>
 							<label for="s0">
 							   <%=biNLS.get("emailactivityname")%></label>
 							 </INPUT>
 							<BR>
 							<BR>
 							<INPUT TYPE=RADIO NAME=rbname id="s1" VALUE=PayA>
							 <label for="s1">
							    <%=biNLS.get("campaignname")%></label>
						        </INPUT>
							<BR>
 							<BR>
 							<INPUT TYPE=RADIO NAME=rbname id="s2" VALUE=PayA>
 							<label for="s2">
 							   <%=biNLS.get("openRate")%></label>
 							 </INPUT>
 							<BR>
 							<BR>
 							<INPUT TYPE=RADIO NAME=rbname id="s3" VALUE=PayA>
							 <label for="s3">
							      <%=biNLS.get("clickThruRate")%></label>
							 </INPUT>
 							<BR>
 							<BR>
 							<INPUT TYPE=RADIO NAME=rbname id="s4" VALUE=PayA>
							  <label for="s4">
							      <%=biNLS.get("shopCartAbandonRate")%></label>
							</INPUT>
 							<BR>
 							<BR>
 							<INPUT TYPE=RADIO NAME=rbname id="s5" VALUE=PayA>
							   <label for="s5">
				                             <%=biNLS.get("optOutRate")%></label>
							</INPUT>
							 <BR>
 							<BR>
 							<INPUT TYPE=RADIO NAME=rbname id="s6" VALUE=PayA>
							   <label for="s6">
                                                            <%=biNLS.get("conversionRate")%></label>
							</INPUT>
							<BR>
 							<BR>
