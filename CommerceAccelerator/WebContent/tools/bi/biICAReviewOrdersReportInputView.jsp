...
<HEAD>
  <link rel=stylesheet href="<%= UIUtil.getCSSFile(biLocale) %>" type="text/css">
   ...
   <TITLE><%=biNLS.get("biICAReviewOrdersReportWindowTitle")%></TITLE>
  ...
  <SCRIPT>
      ...
      {
	   saveStatOption("myHelperReviewOrders");
		setReportFrameworkOutputView("DialogView");
		setReportFrameworkParameter("XMLFile","bi.biICAReviewOrdersReportOutputDialog");
		setReportFrameworkParameter("logon_id",document.LogonId.EnterLogonId.value);
		setReportFrameworkReportXML("bi.biICAReviewOrdersReport");

		if (document.forms["myHelperReviewOrders"].rbsort[0].checked) {
			setReportFrameworkParameter("sortOrder","DESC");
		} else {
			setReportFrameworkParameter("sortOrder","ASC");
		}

		if (document.forms["myHelperReviewOrders"].rbname[0].checked) {
			setReportFrameworkParameter("sortBy","2");
		} else if (document.forms["myHelperReviewOrders"].rbname[1].checked) {
			setReportFrameworkParameter("sortBy","3");
		 }
		
		if(document.LogonId.EnterLogonId.value == "" ){
			setReportFrameworkReportName("biICAReviewOrdersReport");
		}else
			setReportFrameworkReportName("biICAReviewOrdersReportLogonID");
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   function returnSortByOptionLastUpdated(container) {
...
<BODY ONLOAD="initializeValues()" CLASS=content>
   ...
   <H1><%=biNLS.get("biICAReviewOrdersReportWindowTitle") %></H1>
   <%=biNLS.get("biICAReviewOrdersReportDescription")%>
   	...
   	<FORM NAME="LogonId">
		<TABLE CELLPADDING=0 CELLSPACING=0>
         <TR HEIGHT=25>
			...
			<%=biNLS.get("biIndividualReviewReportsEnterLogonId")%> </TD>
            <TD WIDTH=10>&nbsp;</TD>
            <TD COLSPAN=4 VALIGN=TOP>
			<label for="LogonId"> </label>
            <TD><INPUT TYPE=TEXT NAME=EnterLogonId  id="LogonId" SIZE=20 MAXLENGTH=32>&nbsp;</TD>
            </TR>
	       </TABLE>
		</Form>
   </DIV>
   ...
   <p></p>
   <%=biNLS.get("biICAReviewOrdersReportSelection")%>
   <p>
...
<table border="0" bordercolor="black" CELLPADDING="0" CELLSPACING="0">
	...
	<div id=pageBody style="display: block; margin-center:0">
 
<FORM NAME=myHelperReviewOrders>
