...
<link rel=stylesheet href="<%= UIUtil.getCSSFile(biLocale) %>" type="text/css">
   <%=fHeader%>
   <title><LABEL><%=biNLS.get("initiativeDisplayed")%></LABEL></title>
  ...
  <SCRIPT>
       ...
       function initializeValues()
      {
		 ...
		 ResetValues();
		 onLoadStatOption("myHelperInitiativeDisplayed");
      ...
      {
	     saveStatOption("myHelperInitiativeDisplayed");

         setReportFrameworkOutputView("DialogView");
		 setReportFrameworkParameter("XMLFile","bi.biInitiativeDisplayedReportOutputDialog");
         setReportFrameworkReportXML("bi.biInitiativeDisplayedReport");

		  if (document.forms["myHelperInitiativeDisplayed"].rbsort[0].checked) {
				 setReportFrameworkParameter("sortOrder","DESC");
		  } else {
				 setReportFrameworkParameter("sortOrder","ASC");
		  }

		  if (document.forms["myHelperInitiativeDisplayed"].rbname[0].checked) {
				 setReportFrameworkParameter("sortBy","1");
		  } else if (document.forms["myHelperInitiativeDisplayed"].rbname[1].checked) {
				 setReportFrameworkParameter("sortBy","3");
		  } else if (document.forms["myHelperInitiativeDisplayed"].rbname[2].checked) {
				 setReportFrameworkParameter("sortBy","4");
		  } else if (document.forms["myHelperInitiativeDisplayed"].rbname[3].checked) {
				 setReportFrameworkParameter("sortBy","5");
		  }

		  setReportFrameworkReportName("biInitiativeReport");
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   function returnStatOptionDisplayedOrdered(container) {
...
<BODY ONLOAD="initializeValues()" CLASS=content>
   ...
   <h1><LABEL><%=biNLS.get("initiativeDisplayed") %></LABEL></h1>
   <LABEL><%=biNLS.get("initiativeDisplayedDescription") %></LABEL>
 <p>
...
<table border="0" bordercolor="black" CELLPADDING="0" CELLSPACING="0">
	...
	<DIV ID=pageBody STYLE="display: block; margin-left:0">
		<FORM NAME=myHelperInitiativeDisplayed>
					 ...
					 <TR>
						 <TD ALIGN=left VALIGN=TOP>
							...
							<INPUT TYPE=RADIO id="s2" NAME=rbname VALUE=PayA>
								<label for="s2">
								<%=biNLS.get("displayed")%> </label>
							 </INPUT>
							<BR>
							<BR>
							<INPUT TYPE=RADIO id="s3" NAME=rbname VALUE=PayA>
								<label for="s3">
								<%=biNLS.get("clicked")%></label>
							 </INPUT>
							<BR>
							<BR>
							<INPUT TYPE=RADIO id="s4" NAME=rbname VALUE=PayA>
								<label for="s4">
								<%=biNLS.get("clickedOrdered")%> </label>
							 </INPUT>
							<BR>
						 </TD>
					  </TR>
