...
private String generateStatOption(String container, Hashtable biNLS)
{
   String result = 	"<FORM NAME=" + container + ">" +
				    ...
				    "                    " + biNLS.get("biReviewTSRSalesReportOrderByOrderDate") +
				    "			       </INPUT>" +
				    "                  <BR>" +
				    "              </TD>" +
				    "            </TR>" +
				    ...
				    "                 " + biNLS.get("biReviewTSRSalesReportDecend") +
				    "			     </INPUT>" +
				    "                <BR>" +
					"                <BR>" +
				    "                <INPUT TYPE=RADIO NAME=rbsort VALUE=PayA>" +
				    "                 " + biNLS.get("biReviewTSRSalesReportAscend") +
				    "			     </INPUT>" +
				    "                <BR>" +						            		
					"              </TD>" +
					"            </TR>" +
...
<title><LABEL><%=biNLS.get("biReviewTSRSalesReportTitle")%></LABEL></title>
...
<SCRIPT>
       ...
       function initializeValues() 
      {
		 ...
		 ResetValues();
		 onLoadStatOption("myHelperbiReviewTSRSalesReport");
      ...
      function savePanelData() 
      {
		 ...
		 saveStatOption("myHelperbiReviewTSRSalesReport");

         setReportFrameworkOutputView("DialogView");
		 setReportFrameworkParameter("XMLFile","bi.biReviewTSRSalesReportOutputDialog");
         setReportFrameworkReportXML("bi.biReviewTSRSalesReport");

		 myContainer = parent.get("myHelperbiReviewTSRSalesReport",null);
		{	
		 ...
		 if (myContainer == null ||myContainer.TSR_ID==null || myContainer.TSR_ID=="" ){
			 setReportFrameworkReportName("biReviewTSRSalesReportForAll");
			 setReportFrameworkParameter("tsr_id",  null);
		 }else{
			 setReportFrameworkParameter("tsr_id",  myContainer.TSR_ID);
			 setReportFrameworkReportName("biReviewTSRSalesReport");		  
		 }
		}
         saveReportFramework();
	  ...
	  if(document.enquiryPeriod.StartDateEndDateHelperYearSD.value == "" || 
		...
		{
			parent.alertDialog("<%=UIUtil.toJavaScript(biNLS.get("biReviewTSRSalesReportSelectStartEndDate"))%>");
			return false;
...
/*	  if (myContainer == null ||myContainer.TSR_ID==null || myContainer.TSR_ID=="" )
		{
			parent.alertDialog("<%=UIUtil.toJavaScript(biNLS.get("biReviewTSRSalesReportSelectTSRID"))%>");
			return false;
...
<h1><LABEL><%=biNLS.get("biReviewTSRSalesReportTitle")%></LABEL></h1>
<LABEL><%=biNLS.get("biReviewTSRSalesReportInputDescription")%></LABEL>
   <br>
   <br>
	...
	<tr>
		...
		<DIV ID=pageBody STYLE="display: block; margin-left: 0">
			...
			<TABLE CELLPADDING=0 CELLSPACING=0>
				<TR HEIGHT=25>
					<TD VALIGN=TOP><LABEL FOR=CSRId><%=biNLS.get("biReviewTSRSalesReportTSRID")%></LABEL></TD>
				</TR>
				<TR HEIGHT=25>
...
<DIV ID=pageBody STYLE="display: block; margin-left: 0"><%=generateStatOption("myHelperbiReviewTSRSalesReport", biNLS)%>
</DIV>
