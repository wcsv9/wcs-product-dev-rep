...
<link rel=stylesheet href="<%= UIUtil.getCSSFile(biLocale) %>" type="text/css">
   ...
   <title><LABEL><%=biNLS.get("salesByAccount")%></LABEL></title>
  ...
  <SCRIPT>
       ...
       function initializeValues() 
      {
		 ...
		 ResetValues();
		 onLoadStatOption("myHelperSalesByAccount");
 ...
 ///////////////////////////////////////////////////////////////
	...
	function savePanelData() {
		saveStatOption("myHelperSalesByAccount");
		setReportFrameworkOutputView("DialogView");
		setReportFrameworkParameter("XMLFile","bi.biSalesByAccountReportOutputDialog");
		setReportFrameworkReportXML("bi.biSalesByAccountReport");

		if (document.forms["myHelperSalesByAccount"].rbsort[0].checked) {
			setReportFrameworkParameter("sortOrder","DESC");
		} else {
			setReportFrameworkParameter("sortOrder","ASC");
		}

		if (document.forms["myHelperSalesByAccount"].rbname[0].checked) {
			setReportFrameworkParameter("sortBy","1");
		} else if (document.forms["myHelperSalesByAccount"].rbname[1].checked) {
			setReportFrameworkParameter("sortBy","2");
		} else if (document.forms["myHelperSalesByAccount"].rbname[2].checked) {
			setReportFrameworkParameter("sortBy","3");
		} else if (document.forms["myHelperSalesByAccount"].rbname[3].checked) {
			setReportFrameworkParameter("sortBy","4");
		}

		setReportFrameworkReportName("biSalesByAccountReport");
   ...
   function returnStatOptionDayOfAccountName(container) {
...
<BODY ONLOAD="initializeValues()" CLASS=content>
   ...
   <h1><LABEL><%=biNLS.get("salesByAccount") %></LABEL></h1>
   <LABEL><%=biNLS.get("salesByAccountDescription") %></LABEL>
    <p>
...
<DIV ID=pageBody STYLE="display: block; margin-left:0">
   <FORM NAME=myHelperSalesByAccount>
					...
					<TABLE border=0 bordercolor=black CELLPADDING=0 CELLSPACING=0 width=200> 
	            	 ...
	            	 <TD ALIGN=left VALIGN=TOP>
						<TABLE border=0 bordercolor=black CELLPADDING=0 CELLSPACING=0 width=200>
						     <TR HEIGHT=5>
							  <TD ALIGN=left VALIGN=TOP>
						            <INPUT TYPE=RADIO id="ord1" NAME=rbsort VALUE=All id=ord1>									
									<label for="ord1">
						    				<%=biNLS.get("descend")%>
 									</label>
								</INPUT>
						    		 <BR>
							       <BR>
						    		<INPUT TYPE=RADIO id="ord2" NAME=rbsort VALUE=PayA id=ord2%>
								 <label for="ord2">
						    			<%=biNLS.get("ascend")%>
								 </label>
						    		</INPUT>
