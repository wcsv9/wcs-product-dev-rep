...
<link rel=stylesheet href="<%= UIUtil.getCSSFile(biLocale) %>" type="text/css">
   ...
   <title><LABEL><%=biNLS.get("biUASiteActivityByWeekDayReport")%></LABEL></title>
   ...
   <script>
      ...
      function initializeValues()
      {
		 ...
		 onLoadSortOrderByOption("myHelperSiteActivity");
		 ResetValues();
      ...
      {
      	saveStatOption('myHelperSiteActivity');
         setReportFrameworkOutputView("DialogView");
         setReportFrameworkParameter("XMLFile","bi.biUASiteActivityByWeekDayReportOutputDialog");
         setReportFrameworkReportXML("bi.biUASiteActivityByWeekDayReport");

			if (document.forms["myHelperSiteActivity"].rbsort[0].checked) {
				  setReportFrameworkParameter("sortOrder","DESC");
			} else {
				  setReportFrameworkParameter("sortOrder","ASC");
			}

		  if (document.forms["myHelperSiteActivity"].rbname[0].checked) {
					setReportFrameworkParameter("sortBy","1");
		  } else if (document.forms["myHelperSiteActivity"].rbname[1].checked) {
					setReportFrameworkParameter("sortBy","2");
		  } else if (document.forms["myHelperSiteActivity"].rbname[2].checked) {
					setReportFrameworkParameter("sortBy","3");
		  }

		  setReportFrameworkReportName("biUASiteActivityByWeekDayReport");
   ...
   function returnSortByOptionDayofWeek(container) {
      return document.forms[container].rbname[0].checked;
   }
   function returnSortByOptionNumOfShoppers(container) {
      return document.forms[container].rbname[1].checked;
   }
   function returnSortByOptionNumOfVisits(container) {
...
<body onload="initializeValues()" class="content">
	...
	<h1><LABEL><%=biNLS.get("biUASiteActivityByWeekDay") %></LABEL></h1>
	<LABEL><%=biNLS.get("biUASiteActivityByWeekDayDescription") %></LABEL>
	<p></p>
   <LABEL><%=biNLS.get("biUASiteActivityByWeekDayReportSelection")%></LABEL>
   <p>
...
<table border="0" bordercolor="black" CELLPADDING="0" CELLSPACING="0">
	...
	<div id=pageBody style="display: block; margin-left:0">
		<FORM NAME=myHelperSiteActivity>
		      ...
		      <TD ALIGN=left VALIGN=TOP>
				<TABLE border=0 bordercolor=black CELLPADDING=0 CELLSPACING=0 width=200>
		         ...
		         <INPUT TYPE=RADIO NAME=rbname VALUE=All id=s1>
					<label for= s1>
		            <%=biNLS.get("dayofweekSort")%></label>
				 </INPUT>
		         <BR>
		         <BR>
		         <INPUT TYPE=RADIO NAME=rbname VALUE=PayA id=s2>
					<label for= s2>
		            <%=biNLS.get("numOfShoppersSort")%></label>
				 </INPUT>
		         <BR>
		         <BR>
		         <INPUT TYPE=RADIO NAME=rbname VALUE=PayA id=s3>
					<label for= s3>
		            <%=biNLS.get("numOfVisitsSort")%></label>
				 </INPUT>
		         <BR>
		      </TD>
		   </TR>
