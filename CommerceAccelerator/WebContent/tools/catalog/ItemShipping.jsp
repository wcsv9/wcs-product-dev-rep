...
<%
	...
	String itemRefNum = request.getParameter(ECConstants.EC_ITEM_NUMBER); 
        ...
        CalculationCodeSBBean shippingData = null;
      	shippingData = new CalculationCodeSBBean(storeId, itemRefNum, CatalogUtil.SHIPPING_USAGE_ID);      
...
<TITLE><%=UIUtil.toHTML((String)itemResource.get("ShippingTitle"))%></TITLE>
...
<BODY onload="initForm()" class="content">
  ...
  <H1><%=UIUtil.toHTML((String)itemResource.get("ShippingTitle"))%></H1>
  <FORM NAME='shipping'>
  <% if (size == 0) { %>
    <%=itemResource.get("noshipping")%>
  <% } else {%>
    <TABLE BORDER='0'>     
    <TH colspan="3"></TH>   
      <TR>
        <TD><label for='selectedShipping'><%=UIUtil.toHTML((String)itemResource.get("selectedShipping"))%></label></TD>
	<TD WIDTH='20'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</TD>
	<TD><label for='availableShipping'><%=UIUtil.toHTML((String)itemResource.get("availableShipping"))%></label></TD>
      </TR>
	...
	<TD WIDTH='20' VALIGN='TOP'><BR>
	     <INPUT TYPE='button' NAME='addButton' VALUE='<%=UIUtil.toHTML((String)itemResource.get("buttonAdd"))%>' onClick="addToSelectedShipping('<%=UIUtil.toHTML((String)itemResource.get("allshipping"))%>')"><BR>     
	    <INPUT TYPE='button' NAME='removeButton' VALUE='<%=UIUtil.toHTML((String)itemResource.get("buttonRemove"))%>' onClick="removeFromSelectedShipping()">
	</TD>
	<TD>
