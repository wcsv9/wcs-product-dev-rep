...
<%
    try {
   	...
   	String product_id = request.getParameter(ECConstants.EC_PRODUCT_NUMBER); 
        ...
        CalculationCodeSBBean discountData = new CalculationCodeSBBean(storeId, product_id, CatalogUtil.DISCOUNT_USAGE_ID);
...
<TITLE><%=UIUtil.toHTML((String)productResource.get("Discounts"))%></TITLE>
...
<BODY onload="initForm()" class="content">
  ...
  <H1><%=UIUtil.toHTML((String)productResource.get("Discounts"))%></H1>
  <FORM NAME='discount'>
  <% if (size == 0) { %>
    <%=productResource.get("nodiscount")%>
  <% } else {%>
    <TABLE BORDER='0'>     
      <TH colspan="3"></TH>   
      <TR>
        <TD><LABEL for="selectedDiscountsID"><%=UIUtil.toHTML((String)productResource.get("selectedDiscounts"))%></LABEL></TD>
	<TD WIDTH='20'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</TD>
	<TD><LABEL for="availableDiscountsID"><%=UIUtil.toHTML((String)productResource.get("availableDiscounts"))%></LABEL></TD>
      </TR>
	...
	<TD WIDTH='20' VALIGN='TOP'><BR><BR>
	     <INPUT TYPE='button' NAME='addButton' VALUE='<%=UIUtil.toHTML((String)productResource.get("buttonAdd"))%>' onClick="addToAppliedDiscount('<%=UIUtil.toHTML((String)productResource.get("alldiscounts"))%>')"><BR>     
	    <INPUT TYPE='button' NAME='removeButton' VALUE='<%=UIUtil.toHTML((String)productResource.get("buttonRemove"))%>' onClick="removeFromAppliedDiscount()">
	</TD>
	<TD>
