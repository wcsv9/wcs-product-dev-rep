...
<% 
   try {
    ...
    if(product_id != null){
 	catEntImage.setInitKey_catalogEntryReferenceNumber(product_id);
...
<TITLE><%=UIUtil.toHTML((String)productResource.get("image"))%></TITLE>
...
function initForm() {
	...
	document.image.fullimage.select();
        alertDialog("<%= UIUtil.toJavaScript((String)productResource.get("noSpaceForImageLocation"))%>");
...
<H1><%=UIUtil.toHTML((String)productResource.get("image"))%></H1>
<%=UIUtil.toHTML((String)productResource.get("imagePanelInstruction"))%>
<FORM name="image">
...
<TABLE width=500>
    ...
    <TR>
           <TD>
           <LABEL for="fullimageID"><%=UIUtil.toHTML((String)productResource.get("fullimage"))%></LABEL>
       </TD>
    </TR>
    <TR>
           <TD>
    ...
    <TR>
           <TD>
               <LABEL for="thumbnailID"><%=UIUtil.toHTML((String)productResource.get("thumbnail"))%></LABEL>
           </TD>
    </TR>
    <TR>
           <TD>
