...
<%
    try {
   	...
   	String product_id = request.getParameter(ECConstants.EC_PRODUCT_NUMBER); 
        ...
        CalculationCodeSBBean salestaxData = new CalculationCodeSBBean(storeId, product_id, CatalogUtil.SALESTAX_USAGE_ID);
...
<TITLE><%=UIUtil.toHTML((String)productResource.get("SalesTaxTitle"))%></TITLE>
...
<BODY onload="initForm()" class="content">
  ...
  <H1><%=UIUtil.toHTML((String)productResource.get("SalesTaxTitle"))%></H1>
  <FORM NAME='salestax'>
  <% if (size == 0) { %>
    <%=productResource.get("nosalestax")%>
  <% } else {%>
    <TABLE BORDER='0'>     
      <TH colspan="3"></TH>   
      <TR>
        <TD><LABEL for="selectedSalesTaxesID"><%=UIUtil.toHTML((String)productResource.get("selectedSalesTaxes"))%></LABEL></TD>
	<TD WIDTH='20'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</TD>
	<TD><LABEL for="availableSalesTaxesID"><%=UIUtil.toHTML((String)productResource.get("availableSalesTaxes"))%></LABEL></TD>
      </TR>
	...
	<TD WIDTH='20' VALIGN='TOP'><BR>
	     <BUTTON NAME='addButton' onClick="addToSelectedSalesTax('<%=UIUtil.toHTML((String)productResource.get("allsalestaxes"))%>')"><%=UIUtil.toHTML((String)productResource.get("buttonAdd"))%></BUTTON><BR><BR>
	    <BUTTON NAME='removeButton' onClick="removeFromSelectedSalesTax()"><%=UIUtil.toHTML((String)productResource.get("buttonRemove"))%></BUTTON>
	</TD>
	<TD>
