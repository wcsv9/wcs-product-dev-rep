...
<%
    try {
   	...
   	String product_id = request.getParameter(ECConstants.EC_PRODUCT_NUMBER); 
        ...
        CalculationCodeSBBean shippingtaxData = new CalculationCodeSBBean(storeId, product_id, CatalogUtil.SHIPPINGTAX_USAGE_ID);
...
<TITLE><%=UIUtil.toHTML((String)productResource.get("ShippingTaxTitle"))%></TITLE>
...
<BODY onload="initForm()" class="content">
  ...
  <H1><%=UIUtil.toHTML((String)productResource.get("ShippingTaxTitle"))%></H1>
  <FORM NAME='shippingtax'>
  <% if (size == 0) { %>
    <%=productResource.get("noshippingtax")%>
  <% } else {%>
    <TABLE BORDER='0'>     
    <TH colspan="3"></TH>   
      <TR>
        <TD><LABEL for="selectedShippingTaxesID"><%=UIUtil.toHTML((String)productResource.get("selectedShippingTaxes"))%></LABEL></TD>
	<TD WIDTH='20'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</TD>
	<TD><LABEL for="availableShippingTaxesID"><%=UIUtil.toHTML((String)productResource.get("availableShippingTaxes"))%></LABEL></TD>
      </TR>
	...
	<TD WIDTH='20' VALIGN='TOP'><BR>
	    <BUTTON NAME='addButton' onClick="addToSelectedShippingTax('<%=UIUtil.toHTML((String)productResource.get("allshippingtaxes"))%>')"><%=UIUtil.toHTML((String)productResource.get("buttonAdd"))%></BUTTON><BR><BR>
	    <BUTTON NAME='removeButton' onClick="removeFromSelectedShippingTax()"><%=UIUtil.toHTML((String)productResource.get("buttonRemove"))%></BUTTON>
	</TD>
	<TD>
