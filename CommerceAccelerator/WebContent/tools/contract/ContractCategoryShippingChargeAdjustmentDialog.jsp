...
function init(){
   var w = getPageWidth();
   var h = getPageHeight();
   var iframe = parent.document.getElementById("categoryAdjustmentTCIframe");
...
function savePanelData(){
  ...
  if(validatePanelData()){
    parent.document.getElementById("categoryAdjustmentTCIframe").style.visibility = "hidden";
    if(document.getElementById(getCheckboxId(0)).checked) {
      ...
      parent.parent.saveAdjustmentTCInJLOM(lastNodeSelected.value, JROM.FILTER_TYPES[JROM.FILTER_TYPE_CATEGORY], newAdjustmentTC);      
    } else {
      // mark for delete
  ...
  if(document.getElementById(getCheckboxId(0)).checked) {
    ...
    if (adjustType == JROM.ADJUSTMENT_TYPE_AMOUNT_OFF) {
      if (parent.parent.parent.isValidNumber(adjustValue, lang_id, false) == false) {
        ...
        if (parent.document.getElementById("categoryAdjustmentTCIframe").style.visibility == "visible") {
...
function writeTableRows() {
  if (parent.parent.getShipmodePolicyQty()>0) {
    ...
    for (policyId in shipmodes) {
      document.write('<tr>');
      document.write('<td id="CategoryAdjustmentDialog_TableCell_'+j+'1"><input type="checkbox" name="checkbox'+shipmodes[policyId].id+
                     '" id="checkbox' + shipmodes[policyId].id + '" value="false" onclick="validateCheckBox(' + shipmodes[policyId].id + ')"></td>');
      document.write('<td id="CategoryAdjustmentDialog_TableCell_'+j+'2">'+shipmodes[policyId].description + '</td>');
      document.write('<td id="CategoryAdjustmentDialog_TableCell_'+j+'3"><input size="5" maxlength="10" type="text" name="adjustmentValue'+shipmodes[policyId].id+
                     '" id="adjustmentValue'+shipmodes[policyId].id+'">');
      document.write('</td>');
      document.write('<td id="CategoryAdjustmentDialog_TableCell_'+j+'4">' +
...
<table BORDER="0" id="CategoryAdjustmentDialog_Table_1">
<tr>
  <td colspan=4 id="CategoryAdjustmentDialog_TableCell_1">
    <b id="title"><%= contractsRB.get("setShippingChargeAdjustment") %></b>
  </td>
</tr>
<tr>
  <td id="CategoryAdjustmentDialog_TableCell_21">&nbsp;</td>
  <td id="CategoryAdjustmentDialog_TableCell_22"><%= contractsRB.get("shippingModeLabel") %></td>
  <td id="CategoryAdjustmentDialog_TableCell_23"><%= contractsRB.get("amountLabel") %></td>
  <td id="CategoryAdjustmentDialog_TableCell_24"><%= contractsRB.get("typeLabel") %></td>
</tr> 
<tr>
  <td id="CategoryAdjustmentDialog_TableCell_31"><input type="checkbox" name="checkbox0" id="checkbox0" value="false" onclick="validateCheckBox(0)"></td>
  <td id="CategoryAdjustmentDialog_TableCell_32"><label for="checkbox0"><%= contractsRB.get("allShipmodes") %></label></td>
  <td id="CategoryAdjustmentDialog_TableCell_33">
  ...
  <td id="CategoryAdjustmentDialog_TableCell_34">
...
<tr>
   <td COLSPAN=4 ALIGN=RIGHT id="CategoryAdjustmentDialog_TableCell_0">
      <table id="CategoryAdjustmentDialog_Table_2">
         <tr>
            <td id="CategoryAdjustmentDialog_Table_2_Cell_1">
               <button type="button" value="okButton" onclick="savePanelData()" class="general"> <%=UIUtil.toHTML((String)contractsRB.get("ok"))%></button>
            </td>
            <td id="CategoryAdjustmentDialog_Table_2_Cell_2">
