...
//* WebSphere Commerce
//*
//* (c) Copyright IBM Corp. 2001, 2002, 2011
//*
...
<%
	...
	//try to get the customer preferred currency.
...
<form name="itemListForm">
	...
	<%=orderMgmtNLS.get("addProductDialogInstructions") %>
	<br /><br />
	...
	<%= comm.startDlistRowHeading() %>
		...
		<%= comm.addDlistColumnHeading((String)orderMgmtNLS.get("itemUnitPrice"), null, false) %>
	<%= comm.endDlistRow() %>
	...
	for (int i=indexFrom; i<pList.getListSize(); i++) {
		...
		<%= comm.startDlistRow(rowselect) %>
			...
			<input type='hidden' name="SKU_<%=UIUtil.toHTML(pList.getProductListData(i).getCatentryId())%>" value="<%=UIUtil.toHTML(pList.getProductListData(i).getPartNumber())%>" />
			<%= comm.addDlistColumn("<label for='quantity" + i + "'/><input type='text' maxlength='10' name='Quantity_" + pList.getProductListData(i).getCatentryId() + "' id='quantity" + i + "' size='3' align='right' onChange='updateCheckedItem("+pList.getProductListData(i).getCatentryId()+");'>", "none") %>
			...
			<%= comm.addDlistColumn(getFormattedAmount(tradingUnitPrices[0].getValue(), currency, langId, storeId.toString()), "none") %>
		<%= comm.endDlistRow() %>
