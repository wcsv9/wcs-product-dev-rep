...
%>
 <table cellpadding="3" cellspacing="0" border="0" id="StandardCOD_Table_1" >    
	<tr>
		<td colspan="4" valign="middle" id="StandardCOD_TableCell_33">
			<label for="StandardCOD_InputText_3">
				<span class="required">*</span>
				<%= UIUtil.toHTML( (String)resourceBundle.get("paymentInstructionAmount"))%>
			</label>
		</td>
		<td colspan="4" valign="middle" id="StandardCOD_TableCell_34">
			<input type="text" name="piAmount" value="<%=edp_PayMethodAmount%>" id="StandardCOD_InputText_3" />
			<input type=hidden name="paymentTCId" value="<%=paymentTCId%>" />	
			<% if("true".equals(showPONumber)){%> <input type="hidden" name="purchaseorder_id" value="<%=purchaseorder_id%>" id="StandardCOD_InputText_4" /> <%}%> 
		</td>
	</tr>
	<%-- load billing address dropdown box from a seperate page --%>
	<tr>
		<td colspan="4"  valign="middle" id="StandardCOD_TableCell_36">
