...
<script type="text/javascript">
  ...
  var langId= '<%= langId %>';
    var productObjList;
    productObjList = parent.parent.get("<%= RFQConstants.EC_OFFERING_FIXPRICEDYNAMICKIT %>");
...
%>
        ...
        parent.parent.put("<%= RFQConstants.EC_OFFERING_FIXPRICEDYNAMICKIT %>", productObjList);
    }  // end of if ..
	
    function product(reqProductId,reqCatentryid,reqPartNumber,reqProdType,reqName,reqProdDesc,reqCategoryName,reqprice,reqquantity,reqCurrency,reqUnit,reqProdChangeable,productId,catentryid,partNumber,prodType,name,prodDesc,price,quantity,currency,unit,unitDesc) 
    {   
	...
	var form = document.rfqDynamickitFPListForm;
	...
	var url = "/webapp/wcs/tools/servlet/DialogView?XMLFile=rfq.rfqResponseDynamicKitFixedPricingRespondModify";
    	if ( productObjList[index[0]].<%=RFQConstants.EC_RFQ_OFFERING_PRODUCT_CHANGEABLE%> == '<%=RFQConstants.EC_RFQ_PRODUCT_CHANGEABLE_YES%>') 
    	{
	    var rfqId = '<%= RequestId %>';
	    url = "/webapp/wcs/tools/servlet/DialogView?XMLFile=rfq.rfqResponseDynamicKitChangeableFixedPricingRespondModify&amp;requestId=" + rfqId;
        }
	top.saveModel(parent.parent.model);
	top.saveData(productObjList,"allFixedPricingDynamicKits");
...
<%= rfqNLS.get("instruction_DynamicKits_FixedPrice_modify") %>
...
<form name="rfqDynamickitFPListForm" action="">
<script type="text/javascript">
<%
