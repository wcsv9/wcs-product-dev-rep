...
<script type="text/javascript">
    ...
    var productObjList = top.getData("allPercentagePricingDynamicKits", 1);    
	...
	{
	    var unitInfo = document.rfqDynamicKitPPForm.unit[document.rfqDynamicKitPPForm.unit.selectedIndex].value.split(",");
	    ...
	    productObj.<%=RFQConstants.EC_OFFERING_QUANTITY%> = strToNumber(document.rfqDynamicKitPPForm.quantity.value,"<%= langId %>");
	    productObj.product_res_quantity = document.rfqDynamicKitPPForm.quantity.value;
	}

	productObj.<%= RFQConstants.EC_RFQ_CHANGE_STATUS %> = "<%= RFQConstants.EC_RFQ_CHANGE_TRUE %>";
	productObj.<%= RFQConstants.EC_OFFERING_PRICEADJUSTMENT %>=strToNumber('-' + document.rfqDynamicKitPPForm.priceAdjustment.value,<%= langId %>);
    ...
    {
        var priceAdjustment;
        priceAdjustment = productObj.<%= RFQConstants.EC_OFFERING_PRICEADJUSTMENT %>;
       	if (priceAdjustment != null && priceAdjustment != "" && priceAdjustment != undefined) 
       	{
       	    document.rfqDynamicKitPPForm.priceAdjustment.value = numberToStr((priceAdjustment * -1), <%= langId %>, null);
        }
        if (endresult_to_contract == false) 
	{	
            var quantity;
            ...
            if (quantity != null && quantity != "" && quantity != undefined) 
       	    {
        	document.rfqDynamicKitPPForm.quantity.value = productObj.<%=RFQConstants.EC_OFFERING_QUANTITY%>;
            }
        }
    }
