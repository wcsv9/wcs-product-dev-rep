...
<script type="text/javascript">
    ...
    var productObjList = top.getData("allFixedPricingProducts", 1);    
	...
	productObj.<%= RFQConstants.EC_OFFERING_PRICE %> = strToNumber(document.rfqProductForm.price.value,<%= langId %>);
	productObj.<%= RFQConstants.EC_OFFERING_QUANTITY %>=strToNumber( document.rfqProductForm.quantity.value,<%= langId %>);
	productObj.<%= RFQConstants.EC_OFFERING_UNIT %>= document.rfqProductForm.unit.options[document.rfqProductForm.unit.selectedIndex].value;
	productObj.<%= RFQConstants.EC_OFFERING_UNIT_DESC %>= document.rfqProductForm.unit.options[document.rfqProductForm.unit.selectedIndex].text;
    ...
    {
	if (!isValidCurrency(document.rfqProductForm.price.value,productObj.<%= RFQConstants.EC_OFFERING_CURRENCY %>, <%= langId %>)) 
	{
	    alertDialog("<%= UIUtil.toJavaScript(rfqNLS.get("msgInvalidPrice")) %>");
	    document.rfqProductForm.price.focus();
	    return false;
	}
  	var cur2num = "";
	cur2num = parent.currencyToNumber(document.rfqProductForm.price.value, productObj.<%= RFQConstants.EC_OFFERING_CURRENCY %>, <%= langId %>);
	...
	if (!isValidNumber(document.rfqProductForm.quantity.value,<%= langId %>, false)) 
	{
	    alertDialog("<%= UIUtil.toJavaScript(rfqNLS.get("msgInvalidNumber")) %>");
	    document.rfqProductForm.quantity.focus();
	    return false;
	}
    	var resCatentryId = '<%= prodCatentryId %>';
    ...
    function retrievePanelData() 
    {
    	...
    	{
    	    document.rfqProductForm.price.value = numberToCurrency(price, productObj.<%= RFQConstants.EC_OFFERING_CURRENCY %>, <%= langId %>);
        }
       	if (quantity != null && quantity != "" && quantity != undefined) 
       	{
       	    document.rfqProductForm.quantity.value = numberToStr(quantity, <%= langId %>, null);
        }
    }
...
<body class="content" >
<h1><%= rfqNLS.get("rfqresponsefixedprice") %></h1>
<%= rfqNLS.get("instruction_PDresponse_modify") %>

<form name="rfqProductForm" action="">
