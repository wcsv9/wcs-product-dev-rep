...
function loadRegistrationChangeDate () {
	with (document.segmentForm) {
		...
		else if (parent.get && parent.get("invalidRangeRegistrationChangeDateDate", false)) {
			parent.remove("invalidRangeRegistrationChangeDateDate");
			alertDialog("<%= UIUtil.toJavaScript((String) segmentsRB.get("segmentDetailsRegChangeDateInvalidRange")) %>");
			<%= SegmentConstants.ELEMENT_REGISTRATION_CHANGE_DATE_YEAR_1 %>.select();
			<%= SegmentConstants.ELEMENT_REGISTRATION_CHANGE_DATE_YEAR_1 %>.focus();
		}
	}
