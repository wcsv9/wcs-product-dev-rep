...
<%	CalcRuleDetailsDataBean ruleBean = new CalcRuleDetailsDataBean();
	...
	if (scaleMethod != null && ShippingConstants.CALSCALE_CALMETHOD_ID_QUANTITY == scaleMethod.intValue()) {
    ...
    for(var i=0; i < priceRangeList.numberOfRanges(); i++) {
     ...
     if (parent.get) {
				...
				if(o != null){
					...
					selectedMeasureMethod = "<%=ShippingConstants.CALSCALE_CALMETHOD_ID_QUANTITY%>";
					selectedUOM = "C62";
...
function initializeState() 
{
  <% if (priceRanges.size() == 0 ) { %>
	...
	for (var i=0;i<storeCurrencies.length;i++) {
   		...
   		zeroPrices[storeCurrencies[i]] = '0';
  	}
...
try{
  ...
  if(priceRanges == null) priceRanges = new Vector();
...
<h1>
<%=shippingRB.get(ShippingUtil.getMethodCode(new Integer(ShippingConstants.CALRANGE_CALMETHOD_ID_PER_UNIT))) %>
</h1>
<LINE3><%= shippingRB.get("perUnitDesc") %></LINE3>
...
<span id="addCancelButtonSpan" STYLE="display=none">
      <button type="BUTTON"  value="Cancel" name="cancel" id="content" style="border-width:1 1 1 1;text-align:center" onclick="cancelButton();"><%=UIUtil.toHTML((String)shippingRB.get(ShippingConstants.MSG_BUTTON_CANCEL_ADD))%></button>
</span>

<span id="modifyCancelButtonSpan" STYLE="display=none">
      <button type="BUTTON"  value="Cancel" name="cancel" id="content" style="border-width:1 1 1 1;text-align:center" onclick="cancelButton();"><%=UIUtil.toHTML((String)shippingRB.get(ShippingConstants.MSG_BUTTON_CANCEL_MODIFY))%></button>
</span>

<p>
<%=UIUtil.toHTML((String)shippingRB.get("calcScaleRange"))%>
