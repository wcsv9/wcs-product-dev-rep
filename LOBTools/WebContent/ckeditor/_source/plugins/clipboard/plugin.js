...
( function() {
	...
	CKEDITOR.plugins.add( 'clipboard', {
		...
		init: function( editor ) {
			...
			// A paste event listener to remove images with fake path from pasted content in Safari. Ref: #70089
			editor.on( 'paste', function( evt ) {
	            if(CKEDITOR.env.mac && CKEDITOR.env.safari){
	            	var isWebkitFakePathImageRegex = /<img.*src=["']webkit-fake-url[^>]*>/gi,
	                    _html = evt.data.dataValue;
	
	                if (isWebkitFakePathImageRegex.test(_html)){
	                        var webkitFakePathImages = _html.match(isWebkitFakePathImageRegex);

	                        for (var i=0; i<webkitFakePathImages.length; i++){
	                                _html = _html.replace(webkitFakePathImages[i], '');
	                        }
	                        //display error message for user
	                        alert(editor.lang.clipboard.fakepath);
