...
( function() {
	...
	function getBorderWidth( element, side , editor) {
		...
		// IBM Patch for Enhanced Modern Tables
		// RTC #75945
		if( editor && editor.config.ibmModernTable && editor.config.ibmModernTable.enable)
			return Math.max(parseInt( computed, 10 ), 2);
	...
	function buildTableColumnPillars( table , editor) {
		var pillars = [],
			pillarIndex = -1,
		...
		for ( var i = 0, len = $tr.cells.length; i < len; i++ ) {
			...
			rtl ? pillarRight = x + getBorderWidth( td, 'left', editor ) : pillarLeft = x + td.$.offsetWidth - getBorderWidth( td, 'right', editor );

			// Calculate positions based on the next cell, if available.
			if ( nextTd ) {
				x = nextTd.getDocumentPosition().x;

				rtl ? pillarLeft = x + nextTd.$.offsetWidth - getBorderWidth( nextTd, 'right', editor ) : pillarRight = x + getBorderWidth( nextTd, 'left', editor );
			}
		...
		function resizeColumn() {
			...
			for ( var i = 0; i < cellsCount; i++ ) {
				...
				CKEDITOR.tools.setTimeout( function( leftCell, leftOldWidth, rightCell, rightOldWidth, tableWidth, sizeShift ) {
					...
					( !leftCell || !rightCell ) && ( getWidth( table ) + getBorderWidth( table, 'left', editor ) + getBorderWidth( table, 'right', editor ) ),
					currentShift
				] );
			}
		}
			...
			editor.on( 'contentDom', function() {
				...
				editable.attachListener( editable.isInline() ? editable : editor.document, 'mousemove', function( evt ) {
					...
					if ( !( pillars = table.getCustomData( '_cke_table_pillars' ) ) ) {
						...
						table.setCustomData( '_cke_table_pillars', ( pillars = buildTableColumnPillars( table , editor) ) );
