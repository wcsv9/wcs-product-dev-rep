...
<object objectType="eventAndParametersTrigger">
	<parent>
	...
	<c:forEach var="elementVariable" items="${element.campaignElementVariable}">
		<c:if test="${elementVariable.name != 'parameterOneValueList'}">
			<${elementVariable.name}><wcf:cdata data="${elementVariable.value}"/></${elementVariable.name}>
		</c:if>
		<c:if test="${elementVariable.name == 'parameterOneValueList'}">
			<object objectType="parameterOneValue">
				<parameterOneValue><wcf:cdata data="${elementVariable.value}"/></parameterOneValue>
			</object>
		</c:if>
	</c:forEach>
