...
<c:set var="objectType" value="AcctCreditRiskCondition"/>
<c:set var="parent" value="${element.parentElementIdentifier.name}"/>

<c:if test="${empty element.simpleConditionVariable}">
	<c:set var="objectType" value="AcctCreditRiskAndOrList"/>
	<c:set var="parent" value="AcctCreditRisk"/>
	...
	<c:choose>
		...
		<c:when test="${foundChild.simpleConditionOperator == '=' && foundChild.simpleConditionValue == '0'}">
			<c:set var="template" value="equalZero"/>
		</c:when>
		<c:when test="${foundChild.simpleConditionOperator == '=' && foundChild.simpleConditionValue == '100'}">
			<c:set var="template" value="equalOneHundredPercent"/>
		</c:when>
