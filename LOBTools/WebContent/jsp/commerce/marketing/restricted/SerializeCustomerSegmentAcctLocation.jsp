...
<c:choose>
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="AcctLocationList"/>
		<c:set var="parent" value="AcctLocation"/>
	...
	<c:otherwise>
		<c:set var="objectType" value="AcctLocationEqualsOption"/>
		<c:if test="${element.simpleConditionOperator == '!='}" >
			<c:set var="objectType" value="AcctLocationDoesNotEqualOption"/>
		</c:if>
