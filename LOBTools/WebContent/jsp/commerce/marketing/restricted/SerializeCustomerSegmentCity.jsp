...
<c:choose>
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="CityList"/>
		<c:set var="parent" value="City"/>
	...
	<c:otherwise>
		<c:set var="objectType" value="CityEqualsOption"/>
		<c:if test="${element.simpleConditionOperator == '!='}" >
			<c:set var="objectType" value="CityDoesNotEqualOption"/>
		</c:if>
