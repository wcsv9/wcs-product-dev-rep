...
<c:choose>
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="CountryRegionList"/>
		<c:set var="parent" value="CountryRegion"/>
	...
	<c:otherwise>
		<c:set var="objectType" value="CountryRegionEqualsOption"/>
		<c:if test="${element.simpleConditionOperator == '!='}" >
			<c:set var="objectType" value="CountryRegionDoesNotEqualOption"/>
		</c:if>
