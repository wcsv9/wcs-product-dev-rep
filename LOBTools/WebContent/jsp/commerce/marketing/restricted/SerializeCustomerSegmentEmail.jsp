...
<c:choose>
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="EmailList"/>
		<c:set var="parent" value="Email"/>
	...
	<c:otherwise>
		<c:set var="objectType" value="EmailEndsWithOption"/>
		<c:if test="${element.simpleConditionOperator == 'doesNotEndWith'}" >
			<c:set var="objectType" value="EmailDoesNotEndWithOption"/>
		</c:if>
