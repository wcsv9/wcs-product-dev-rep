...
<c:set var="objectType" value="GenderCondition"/>
<c:set var="parent" value="${element.parentElementIdentifier.name}"/>

<c:if test="${empty element.simpleConditionVariable}">
	<c:set var="objectType" value="GenderOrList"/>
	<c:set var="parent" value="Gender"/>

	<object objectType="Gender">
		<parent>
			<object objectId="${element.parentElementIdentifier.name}"/>
		</parent>
		<elementName>Gender</elementName>
