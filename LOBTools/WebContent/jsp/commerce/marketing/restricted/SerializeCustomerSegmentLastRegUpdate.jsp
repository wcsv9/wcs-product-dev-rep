...
<c:choose>
	...
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="LastRegistrationUpdateAndOrList"/>
		<c:set var="parent" value="LastRegistrationUpdate"/>
		...
		<c:choose>
			...
			<c:when test="${foundChild.simpleConditionOperator == '>' && foundChild.simpleConditionVariable == 'daysSinceRegistrationChange'}">
				<c:set var="template" value="priorDays"/>
			</c:when>
			<c:when test="${foundChild.simpleConditionOperator == '>' && foundChild.simpleConditionVariable == 'registrationChangeDate'}">
	...
	<c:otherwise>
		<c:choose>
			<c:when test="${element.simpleConditionVariable == 'daysSinceRegistrationChange'}">
				<c:set var="objectType" value="LastRegUpdateDaysCondition"/>
			</c:when>
			<c:when test="${element.simpleConditionVariable == 'registrationChangeDate'}">
				<c:set var="objectType" value="LastRegUpdateDatesCondition"/>
			</c:when>
		</c:choose>
	</c:otherwise>
