...
<c:choose>
	...
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="LastVisitDateAndOrList"/>
		<c:set var="parent" value="LastVisitDate"/>
		...
		<c:choose>
			...
			<c:when test="${foundChild.simpleConditionOperator == '>' && foundChild.simpleConditionVariable == 'daysSinceLastVisit'}">
				<c:set var="template" value="priorDays"/>
			</c:when>
			<c:when test="${foundChild.simpleConditionOperator == '>' && foundChild.simpleConditionVariable == 'lastVisitDate'}">
	...
	<c:otherwise>
		<c:choose>
			<c:when test="${element.simpleConditionVariable == 'daysSinceLastVisit'}">
				<c:set var="objectType" value="LastVisitDaysCondition"/>
			</c:when>
			<c:when test="${element.simpleConditionVariable == 'lastVisitDate'}">
				<c:set var="objectType" value="LastVisitDatesCondition"/>
			</c:when>
		</c:choose>
	</c:otherwise>
