...
<c:set var="objectType" value="OrderFulfilledCondition"/>
<c:set var="parent" value="${element.parentElementIdentifier.name}"/>

<c:if test="${empty element.simpleConditionVariable}">
	<c:set var="objectType" value="OrderFulfilledAndOrList"/>
	<c:set var="parent" value="OrderFulfilled"/>
	...
	<c:choose>
		...
		<c:when test="${foundChild.simpleConditionOperator == '<='}">
			<c:set var="template" value="lessOrEqual"/>
		</c:when>
		<c:otherwise>
