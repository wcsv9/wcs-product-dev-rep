...
<c:choose>
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="PhoneNumberList"/>
		<c:set var="parent" value="PhoneNumber"/>
	...
	<c:otherwise>
		<c:set var="objectType" value="PhoneNumberStartsWithOption"/>
		<c:if test="${element.simpleConditionOperator == 'doesNotStartWith'}" >
			<c:set var="objectType" value="PhoneNumberNotStartsWithOption"/>
		</c:if>
