...
<c:choose>
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="StateProvinceList"/>
		<c:set var="parent" value="StateProvince"/>
	...
	<c:otherwise>
		<c:set var="objectType" value="StateProvinceEqualsOption"/>
		<c:if test="${element.simpleConditionOperator == '!='}" >
			<c:set var="objectType" value="StateProvinceDoesNotEqualOption"/>
		</c:if>
