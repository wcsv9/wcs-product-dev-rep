...
<c:choose>
	<c:when test="${empty element.simpleConditionVariable}">
		<c:set var="objectType" value="ZipCodeList"/>
		<c:set var="parent" value="ZipCode"/>
	...
	<c:otherwise>
		<c:set var="objectType" value="ZipCodeStartsWithOption"/>
		<c:if test="${element.simpleConditionOperator == 'doesNotStartWith'}" >
			<c:set var="objectType" value="ZipCodeDoesNotStartWithOption"/>
		</c:if>
