	...
	<c:if test="${!(empty storeIdList)}">
		<c:forEach var="currentStoreId" items="${storeIdList}">
			...
			<wcf:getData type="com.ibm.commerce.pagelayout.facade.datatypes.LayoutType"
				var="pagelayout" 
				expressionBuilder="getLayoutsByUniqueID">
				<wcf:contextData name="storeId" data="${param.storeId}" />
				<wcf:param name="accessProfile" value="IBM_Admin_Details"/>
				<wcf:param name="layoutId" value="${param.pageLayoutId}"/>
			</wcf:getData>

			<%-- Find all 'Content' or 'Category' or 'Product/Item/Kit/Bundle/DynamicKit/PredDynaKit' page locations --%>
			<c:if test="${pagelayout != null && !(empty pagelayout.pageLocation)}">
				<c:forEach var="pagelocation" items="${pagelayout.pageLocation}">
					<c:if test="${pagelocation != null}">
						<c:set var="pageGroup" value="${pagelocation.page.pageGroup}"/>
						<c:if test="${pageGroup == 'Content' || pageGroup == 'Category' || pageGroup == 'Product'
						|| pageGroup == 'Kit' || pageGroup == 'DynamicKit' || pageGroup == 'PredDynaKit' || pageGroup == 'Item' || pageGroup == 'Bundle'}">
							<jsp:directive.include file="serialize/SerializePageLayoutLocationPreviewUrl.jspf"/> 
						</c:if>
					</c:if>
				</c:forEach>
			</c:if>
