...
<object objectType="externalRule">
	...
	<c:forEach var="elementVariable" items="${element.elementAttribute}">
		<c:choose>
			<c:when test="${elementVariable.name == 'externalRuleIdentifier'}">
				<c:set var="externalRuleIdentifier" value="${elementVariable.value}" />
			</c:when>
			<c:otherwise>
	...
	<c:if test="${externalRuleIdentifier != ''}">
		<object objectType="RefExternalPriceRule">
			<externalRuleIdentifier><wcf:cdata data="${externalRuleIdentifier}" /></externalRuleIdentifier>
			<object objectType="ExternalPriceRule">
				<externalRuleIdentifier><wcf:cdata data="${externalRuleIdentifier}" /></externalRuleIdentifier>
			</object>
		</object>
	</c:if>
