            ...
            <%@ include file="ContentRecommendation_Data.jspf" %>
                ...
                <c:if test="${env_inPreview && !env_storePreviewLink && ( empty ignorePreview )}">
                    ...
                    <c:if test="${emsName == 'SearchResults_Content'}">
                        <%-- refresh area for eSpost 'SearchResults_Content' preview popup only --%>
                            <wcf:url var="SearchRuleInfoViewURL" value="SearchRuleInfoViewV2" type="Ajax">
                            ...
                            <script type="text/javascript">
                                function SearchRuleInfo(widgetSuffix) {
                                    var myRCProperties = wcRenderContext.getRenderContextProperties("searchBasedNavigation_context"),
                                        myWidgetObj = $("#SearchRuleInfo" + widgetSuffix),
                                        renderContextChangedHandler = function() {
                                            myWidgetObj.refreshWidget("refresh", params);
                                        };
                            ...
                            <div wcType="RefreshArea" id="SearchRuleInfo${widgetSuffix}" declareFunction="searchRuleInfo('${widgetSuffix}')" refreshurl="${SearchRuleInfoViewURL}" ariaMessage="<wcst:message key='ACCE_STATUS_SEARCHRULEINFORMATION_UPDATED' bundle='${widgetText}' />" ariaLiveId="${ariaMessageNode}" role="region" aria-label="<wcst:message key='SEARCH_RULE_INFO_PREVIEW_POPUP_ACCE_Label' bundle='${widgetText}' />">
                    </c:if>
                ...
                <flow:ifEnabled feature="CDNCaching">
                    ...
                    <c:if test="${eSpotDatas.behavior != 0 && empty param.dontCreateRefreshArea}">
                        ...
                        <c:set var="recommendationType" value="content" scope="request"/>
                        <%@include file="/Widgets_801/Common/ESpot/AjaxRecommendation_UI.jspf"%>
                    </c:if>
                </flow:ifEnabled>

                <c:if test="${loadContent eq 'true' && !empty contentFormatMap}">
