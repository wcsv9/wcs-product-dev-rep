...
<flow:ifEnabled feature="CDNCaching">
	...
	An activity behavior flag from of 0 means static, whereas an activity behavior of 1 or 2 means dynamic.
	--%>
	<c:if test="${eSpotDatas.behavior != 0 && empty param.dontCreateRefreshArea}">
		<%-- Make an Ajax request to get the dynamic content if no Ajax request has been made for this eSpot. --%>
		<c:set var="loadContent" value="false" />
		<c:set var="recommendationType" value="espot" scope="request"/>
		<%@include file="/Widgets_801/Common/ESpot/AjaxRecommendation_UI.jspf"%>
	</c:if>
</flow:ifEnabled>

<c:if test = "${loadContent eq 'true'}">
