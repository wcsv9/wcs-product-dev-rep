...
<c:if test="${!empty param.includedMemberGroupDisplayNames}">
	<c:set var="mbrGroupIncludedNames" value="${param.includedMemberGroupDisplayNames }"/>
</c:if>
...
<wcf:rest var="searchIndexting" url="store/{storeId}/searchindex" cached="true" scope="request">
	...
	<wcf:param name="workspaceId" value="${workspaceId }" />
</wcf:rest>
			...
			function showGeneratedURLPopup(generatedURLData) {
				...
				if (storeFrame != null) {
					if (storeFrame.document.getElementById("generatedURLDialog") != null) {
						...
						var generatedURL = currentURL.protocol + "//" + currentURL.host + currentURL.pathname + currentURL.search + (currentURL.search == "" ? "?" : "&") + "newPreviewSession=true&previewToken=" + encodeURIComponent(generatedURLData.previewToken);
						if (generatedURLData.includedMemberGroupIds){
							generatedURL = generatedURL + "&includedMemberGroupIds=" + encodeURIComponent(generatedURLData.includedMemberGroupIds);
						}
						if (generatedURLData.includedMemberGroupDisplayNames){
							generatedURL = generatedURL + "&includedMemberGroupDisplayNames=" + generatedURLData.includedMemberGroupDisplayNames;
						}
						if (generatedURLData.locale){
							generatedURL = generatedURL + "&toolsLocale=" + generatedURLData.locale;
						}
						generatedURL = generatedURL + currentURL.hash;
						var qrImg = null;
						try {
							var qr = qrcode(10, "L");
				...
				if (true == validate()) {
					$.ajax({
						...
						data: {
							...
							<c:if test="${!empty param.includedMemberGroupIds}">includedMemberGroupIds: "<c:out value='${param.includedMemberGroupIds}'/>",
							</c:if><c:if test="${!empty param.includedMemberGroupDisplayNames}">includedMemberGroupDisplayNames: encodeURIComponent("<c:out value='${param.includedMemberGroupDisplayNames}'/>"),
							</c:if><c:if test="${workspaceId != null && !empty workspaceId}">workspaceId: "<c:out value='${workspaceId}'/>",
							</c:if><c:if test="${!empty param.locale}">locale: "<c:out value='${param.locale}'/>",
							</c:if>tokenLife: token_life,
							password: pword,
		     ...
		     <div class="details">
				...
				<div class="content" id="detailsSection">
                    ...
                    <c:if test="${!empty param.includedMemberGroupIds}">
                    	<span class="highlight"><fmt:message key="storePreviewCustomerSegments" bundle="${resources}" /></span>&nbsp;${mbrGroupIncludedNames} <br/>
		    		</c:if>
