				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td><a class="button primary"
							onClick="javascript:ShipmodeSelectionExtJS.checkPartNumber('AddItemToCartForm','product')"
					...
					<div id="checkout-button-row">
						...
						<c:if test="${userType eq 'G'}">
							<div class="button-blue">
								<input type="hidden" name="guestCheckLogon" id="guestCheckLogon" value="true"/>
										...
										<c:choose>
											 <c:when test="${!empty sessionScope.redPoint}">
												<div class="button_align left" id="WC_CheckoutLogonf_div_10">
													<a href="#" role="button" class="button_primary" id="shopcartCheckout" tabindex="0" onclick="javascript:TealeafWCJS.processDOMEvent(event);if(CheckoutHelperJS.checkRedeem('<c:out value="${sessionScope.totalPoints}"/>','<c:out value="${sessionScope.redPoint}"/>','0') && CheckoutHelperJS.canCheckoutContinue('<c:out value="${userType}"/>') && CheckoutHelperJS.updateShoppingCart(document.ShopCartForm,true)){ShipmodeSelectionExtJS.registeredUserContinue('<c:out value='${OrderCalculateURL}'/>', '<c:out value='${PhysicalStoreSelectionURL}'/>','','','<c:out value='${orderId}'/>');}return false;">
													<c:remove var="redPoint"/>
											</c:when>
											<c:when test="${priceZeroCheck eq 'true' && sessionScope.loyaltyEnabled ne 'true' && proceed eq 'true'}">
												<div class="button_align left" id="WC_CheckoutLogonf_div_10">
											...
											<c:otherwise>
												...
												<c:if test="${userType ne 'G'}">
													...
													<%-- <a href="#" role="button" class="button_primary" id="shopcartCheckout" tabindex="0" onclick="javascript:TealeafWCJS.processDOMEvent(event);if(CheckoutHelperJS.clickOnCheckout()){if(CheckoutHelperJS.checkRedeem('<c:out value="${sessionScope.totalPoints}"/>','<c:out value="${sessionScope.redPoint}"/>','0') && CheckoutHelperJS.canCheckoutContinue('<c:out value="${userType}"/>') && CheckoutHelperJS.updateShoppingCart(document.ShopCartForm,true)){ShipmodeSelectionExtJS.registeredUserContinue('<c:out value='${OrderCalculateURL}'/>', '<c:out value='${PhysicalStoreSelectionURL}'/>','','','<c:out value='${orderId}'/>');}return false;}">--%>
												</c:if>
												<c:if test="${userType eq 'G'}">
