...
<div id="entitledItem_<c:out value='${catalogEntryID}'/>" style="display:none;">
		...
		<c:set var="splt" value="${fn:split(catalogEntryDetails.parentCatalogGroupID[0],'_')}"/>
		...
		<input type="hidden" value='<c:out value="${category.catalogGroupView[0].longDescription }"/>' id='itemCategory_<c:out value="${catalogEntryDetails.uniqueID}"/>'/>
	</body>
<script type="text/javascript">
	...
	var cat = '';
	if (document.getElementById('itemCategory_'+ productDetails.uniqueID)) {
		cat = document.getElementById('itemCategory_'+ productDetails.uniqueID).value;
	}
	var product = {
		'name': productDetails.name,
		'id': productDetails.partNumber, //SKU
		'price': document.getElementById('ProductInfoPrice_'+ productDetails.uniqueID).value.replace('$', ''),
		'brand': brand,
		'variant': '',
		'category': cat
	};
