...
<html xmlns="http://www.w3.org/1999/xhtml" lang="${shortLocale}" xml:lang="${shortLocale}">
	...
	<body onload="hideDivs();">
		...
		<div id="page" class="nonRWDPage">
			...
			<script>	
				function PrintMe(){
					var dispSetting="width=1200,height=800,left=100,top=25,scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no,copyhistory=no,resizable=yes";
					var url = window.location.href;
					if (url.endsWith('#')) {
						url = url.substring(0, window.location.href.length-1);
					}
					var win = window.open(url+'&inPopup=true#',"",dispSetting);
				}
				
				function hideDivs() {
					var url = new URL(window.location.href);
					var c = url.searchParams.get("inPopup");
					if (c) {
						document.getElementById('headerRow2').style.display = 'none';
						document.getElementById('quickLinksBar').style.display = 'none';
			...
			<!-- Main Content Start -->
			<div class="content_wrapper_position v9ConfirmationPage" role="main">
								...
								<div class="container_full_width">
													...
													<c:choose>
														...
														<c:otherwise>
															<c:choose>
																...
																<c:otherwise>
																	<h1 class="breadcrumb_current"><%-- <fmt:message bundle="${storeText}" key="ORD_THANKS_MESSAGE"/> --%>
																		Thank you for shopping with ${storeContact.stDisplayName}.
																		<br />
																		The Order Number is ${order.orderId}
																	</h1>
																	<%-- <p><fmt:message bundle="${storeText}" key="ORD_CONFIRMATION_MESSAGE"/></p> --%>
																	<p class="orderSubmitted"> <strong>Your order has been submitted.</strong></p>
																	<p>	<span class="receivedEmail">You will now receive an email of the order.</span></p>
                                                						<br />
									...
									<div id="box">
															...
															<div class="orderExpandArea">
																<span id="OrderItemsExpandArea">
																	<a id="OrderItemDetails_plusImage_link" class="tlignore" href="javaScript:CheckoutHelperJS.toggleOrderItemDetailsSummary('OrderConfirmPagingDisplay', 'OrderItemPaginationDisplay_Context', '<c:out value='${beginIndex + pageSize}'/>', '<c:out value='${pageSize}'/>', '${WCParam.externalOrderId}', '${WCParam.externalQuoteId}', 'false'<flow:ifEnabled feature="Analytics">,'true'</flow:ifEnabled>);" onclick="javaScript:TealeafWCJS.processDOMEvent(event);">
																		<img id="OrderItemDetailsPlus" style="display:inline" src="<c:out value='${jspStoreImgDir}images/'/>icon_plusBlue.png">
																	...
																	<a id="OrderItemDetails_minusImage_link" style="display:none" class="tlignore" href="javaScript:CheckoutHelperJS.toggleOrderItemDetailsSummary('OrderConfirmPagingDisplay', 'OrderItemPaginationDisplay_Context', '<c:out value='${beginIndex + pageSize}'/>', '<c:out value='${pageSize}'/>', '${WCParam.externalOrderId}', '${WCParam.externalQuoteId}', 'false'<flow:ifEnabled feature="Analytics">,'true'</flow:ifEnabled>);" onclick="Javascript:TealeafWCJS.processDOMEvent(event)">
																		<img id="OrderItemDetailsMinus" style="display:none" src="<c:out value='${jspStoreImgDir}images/'/>icon_minusBlue.png">
										...
										<div class="left left_confirmation_page" id="WC_OrderShippingBillingConfirmationPage_div_32">
											<a href="#" role="button" class="button_primary" id="WC_OrderShippingBillingConfirmationPage_Print_Link" tabindex="0" onclick="JavaScript: PrintMe();">
												<div class="left_border"></div>
		...
		<%out.flush();%>
			<c:import url ="${env_jspStoreDir}/ShoppingArea/CheckoutSection/GmtPurchaseData.jsp">
				<c:param name="catalogId" value="${WCParam.catalogId}" />
					<c:param name="pageSize" value="${pageSize}"/>
					<c:param name="pageNumber" value="${currentPage}"/>
					<c:param name="searchHostNamePath" value="${searchHostNamePath}"/>
					<c:param name="searchContextPath" value="${searchContextPath}"/>
					<c:param name="envCurrencyCode" value="${env_currencyCode}"/>
					<c:param name="catagId" value="${sdb.masterCatalog.catalogId}"/>
					<c:param name="url" value="${env_jspStoreDir}"/>
				</c:import>
		<%out.flush();%>
		<flow:ifEnabled feature="Analytics">
