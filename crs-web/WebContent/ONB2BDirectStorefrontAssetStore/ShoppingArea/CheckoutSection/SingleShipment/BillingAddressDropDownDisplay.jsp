						...
						<c:choose>
							<c:when test="${payment.paymentMethodName eq paymentMethodSelected}">
								<c:forEach var="addressInPayment" items="${payment.usableBillingAddress}">
									<wcf:rest var="flag" url="store/{storeId}/userRegister/getPersonActiveFlag/{addressId}">
									      <wcf:var name="storeId" value="${WCParam.storeId}" encode="true"/>
									      <wcf:var name="addressId" value="${addressInPayment.addressId}" encode="true"/>
									</wcf:rest>
									...
									<c:choose>
										<c:when test="${flag.message eq 'true' || flag.message eq 'true1' || userType eq 'G'}">
											<c:set var="selectedAddressId" value="${addressInPayment.addressId}"/>
											<c:if test="${addressInPayment.addressId == selectedAddressId}">
												<c:set var="selectStr" value='selected="selected"' />
											</c:if>
											
											<option <c:out value="${selectStr}" escapeXml='false'/> value="${addressInPayment.addressId}">
											<c:choose>
											...
											<c:otherwise><c:out value="${addressInPayment.nickName}"/></c:otherwise></c:choose></option>
