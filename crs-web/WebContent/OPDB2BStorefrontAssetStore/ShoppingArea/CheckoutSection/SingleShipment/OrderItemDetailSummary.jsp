...
<c:if test="${empty subscriptionOrderItemId}">
 ...
 <table id="order_details" class="v9_order_details" cellpadding="0" cellspacing="0" border="0" width="100%" summary="<fmt:message bundle="${storeText}" key="SHOPCART_TABLE_CONFIRM_SUMMARY"/>">
	...
	<tr class="nested">
		...
		<c:set var="th_count" value="${th_count + 1}"/>
		<th class="each short" id="SingleShipment_tableCell_unitPriceExtGst" abbr="Each <br> (Ex GST)">Each <br> (Ex GST)</th>
		<c:set var="th_count" value="${th_count + 1}"/>
	...
	<c:forEach var="orderItem" items="${pagorder.orderItem}" varStatus="status">
		...
		<tr>
			<td class="th_align_left_normal ${fn:escapeXml(nobottom)}" id="SingleShipment_rowHeader_product${fn:escapeXml(status.count)}" abbr="<fmt:message bundle="${storeText}" key="Checkout_ACCE_for"/> <c:out value='${catEntry.name}' escapeXml='${env_escapeXmlFlag}'/>">
				<div class="row col12">
				<div class="img col2 acol12 setWidthIpad" id="WC_OrderItemDetailsSummaryf_div_1_${fn:escapeXml(status.count)}">
					<c:choose>
						<c:when test="${!empty thumbNail}">
							<img src="http://s3-ap-southeast-2.amazonaws.com/obv9.public${fn:escapeXml(thumbNail)}" alt="${fn:escapeXml(catEntry.name)}" />
						</c:when>
						<c:otherwise>
							<img src="http://s3-ap-southeast-2.amazonaws.com/obv9.public${fn:escapeXml(jspStoreImgDir)}images/NoImageIcon_sm.jpg" alt="${fn:escapeXml(catEntry.name)}" />
						</c:otherwise>
					</c:choose>
					<c:remove var="thumbNail"/>
				</div>
				<div class="itemspecs col8 acol12" id="WC_OrderItemDetailsSummaryf_div_2_${fn:escapeXml(status.count)}">
