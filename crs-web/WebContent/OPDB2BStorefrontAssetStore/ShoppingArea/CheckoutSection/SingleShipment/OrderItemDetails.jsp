...
<input type="hidden" name="currentPageNumber" value="${currentPage}" id="currentPageNumber"/>
 ...
 <table id="order_details" class="tblProducts" cellpadding="0" cellspacing="0" border="0" width="100%" summary="<fmt:message bundle="${storeText}" key="SHOPCART_TABLE_CONFIRM_SUMMARY"  />">
	<tr class="nested">
		...
		<th class="align_right" id="shoppingCart_tableCell_extGst">Each<br>(Ex GST)</th>
		<th class="align_right" id="shoppingCart_tableCell_each" abbr="<fmt:message bundle="${storeText}" key="UNIT_PRICE" />">Each<br>(Inc. GST)</th>
		...
		<th class="total short" id="SingleShipment_tableCell_totalPrice" abbr="<fmt:message bundle="${storeText}" key="TOTAL_PRICE"  />"><fmt:message bundle="${storeText}" key="TOTAL"  /><br>(Inc. GST)</th>
	</tr>
	...
	<c:forEach var="orderItem" items="${pgorder.orderItem}" varStatus="status">
		...
		<tr>
			<td class="th_align_left_normal <c:out value="${nobottom}"/>" id="SingleShipment_rowHeader_product<c:out value='${status.count}'/>" abbr="<fmt:message bundle="${storeText}" key="Checkout_ACCE_for"  /> <c:out value='${catEntry.name}' escapeXml='${env_escapeXmlFlag}'/>">
				<div class="row col12">
					<div class="col2 acol12 setWidthIpad">
						...
						<a href="${catEntryDisplayUrl}" id="catalogEntry_img_${orderItem.orderItemId}" title="<c:out value='${catEntry.name}' escapeXml='${env_escapeXmlFlag}'/>">
							<img alt="<c:out value='${catEntry.name}' escapeXml='${env_escapeXmlFlag}'/>" src="${s3ImageHost}${imgSource}"/>
						</a>
				...
				<div class="col8 acol12">
					<div class="itemspecs" id="WC_OrderItemDetails_div_2_<c:out value='${status.count}'/>">
						...
						<span>Product Code:<c:out value="${catEntry.partNumber}" escapeXml='${env_escapeXmlFlag}'/>						
						</span>
						<br />
						...
						<c:if test="${!isFreeGift}">
								...
								<flow:ifEnabled feature="RequisitionList">
									...
									<input type="hidden" id="ProductInfo_PageName" value="OrderItemDetailsPage"/>												
									<script type="text/javascript">
										$(document).ready(function() {									
											wcTopic.subscribe("DefiningAttributes_Resolved<c:out value='${orderItem.orderItemId}'/>", addReqListsJS<c:out value='${orderItem.orderItemId}'/>.setCatEntryId);																		
											wcTopic.publish("DefiningAttributes_Resolved<c:out value='${orderItem.orderItemId}'/>", "<c:out value='${orderItem.productId}'/>", "<c:out value='${parentCatEntryId}'/>");
				...
				<p class="item-quantity">
					<c:choose>
						...
						<c:otherwise>
							...
							<input type="hidden" value="<c:out value="${quickCartOrderItemQuantity}"/>" id='quantity_<c:out value="${orderItem.productId}"/>' name='quantity_<c:out value="${orderItem.productId}"/>' />
						</c:otherwise>
					</c:choose>
				</p>
			</td>
