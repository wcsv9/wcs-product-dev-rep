...
<div class="label_spacer" id="WC_${formName}_country_div_17">
	<span class="spanacce"> <label
		for="<c:out value='WC_${pageName}_ShoppingCartAddressEntryForm_${formName}_${paramPrefix}country_1'/>">
			<fmt:message bundle="${storeText}"
				key="SHIPPING_AND_BILLING_ADDRESS_LABEL_TEXT_COUNTRY2_ACCE">
				<fmt:param>${address}</fmt:param>
			</fmt:message>
	</label>
	</span> <span class="required-field" id="WC_${formName}_country_div_18">
		*</span>
	<fmt:message bundle="${storeText}" key="COUNTRY2" />
</div>
<div id="WC_${formName}_country_div_19">
	<c:choose>
		<c:when test="${storeId eq '81852'}">
			<c:set var="lang" value="${locale}" />
			<c:set var="country_language_select" value="${fn:split(lang, '_')}" />
			<select aria-required="true" name="country" size="1"
				id="<c:out value='WC_${pageName}_ShoppingCartAddressEntryForm_${formName}_${paramPrefix}country_1'/>"
				class="drop_down_checkout" onkeydown="saveTabPress(event)"
				onblur="setFocus('WC_${pageName}_ShoppingCartAddressEntryForm_${formName}_${paramPrefix}state_1')"
				onchange="javascript:AddressHelper.loadStatesUI('<c:out value='${formName}'/>','<c:out value='${paramPrefix}'/>','<c:out value='${paramPrefix}stateDiv${divNum}'/>','<c:out value="WC_${pageName}_ShoppingCartAddressEntryForm_${formName}_${paramPrefix}state_1"/>')">
				<c:forEach var="country" items="${countryBean.countries}">
					<c:if test="${country.displayName eq 'Australia'}">
		...
		<c:otherwise>
			...
			<select aria-required="true" name="country" size="1"
				...
				<c:forEach var="country" items="${countryBean.countries}">
					<c:if
						test="${country.displayName eq 'Australia' || country.displayName eq 'New Zealand'}">
						<option value="<c:out value='${country.code}'/>"
							<c:choose>
								<c:when test="${!empty contact.country}">
									<c:if test="${country.code eq contact.country || country.displayName eq contact.country}">
...
<span class="required-field"  id="WC_${formName}_phone1_div_222"> *</span>
