...
<div class="reward-points">
	Welcome back
	<c:out value="${getLoyaltyRestTransactions.firstName}" />
	&nbsp;
	<c:out value="${getLoyaltyRestTransactions.firstName}" />
	! I see you`ve got
	<%--
	...
	<c:choose>
		<c:when test="${getLoyaltyRestTransactions.response eq 'no response' }">
			<span class="reward-points-no"><span class="star">0</span></span>
		</c:when>
		<c:otherwise>
			<c:forEach var="strArray" items="${getLoyaltyRestTransactions.strArray}" varStatus="status">
...
<table cellpadding="0" class="rewardPointsTbl" cellspacing="0" border="0" width="100%">
	...
	<c:choose>
		...
		<c:otherwise>
			<c:forEach var="transaction" items="${getLoyaltyRestTransactions.loyaltyTransactions}" varStatus="status">
