...
<wcf:url var="AjaxProvinceSelectionDisplayURL" value="SLProvinceSelectionDisplayViewV2" type="Ajax">
	<wcf:param name="storeId" value="${param.storeId}" />
	<wcf:param name="catalogId" value="${param.catalogId}" />
	<wcf:param name="langId" value="${langId}" />
	<wcf:param name="ajaxStoreImageDir" value="${jspStoreImgDir}" />
</wcf:url>
<wcf:url var="AjaxCitySelectionDisplayURL" value="SLCitySelectionDisplayViewV2" type="Ajax">
	<wcf:param name="storeId" value="${param.storeId}" />
	<wcf:param name="catalogId" value="${param.catalogId}" />
	<wcf:param name="langId" value="${langId}" />
<wcf:param name="ajaxStoreImageDir" value="${jspStoreImgDir}" />	
</wcf:url>
<wcf:url var="AjaxStoreLocatorResultsURL" value="SLResultsViewV2" type="Ajax">
	<wcf:param name="storeId" value="${param.storeId}" />
	<wcf:param name="catalogId" value="${param.catalogId}" />
...
<wcf:url var="AjaxSelectedStoreListURL" value="SLSelectedStoreListViewV2" type="Ajax">
	<wcf:param name="storeId" value="${param.storeId}" />
	<wcf:param name="catalogId" value="${param.catalogId}" />
	<wcf:param name="orderId" value="${param.orderId}" />
	<wcf:param name="langId" value="${langId}" />
...
<script type="text/javascript">
	function openStoreLocatorPopup() {
		var storeLocatorPopup = $("#storeLocatorPopup").data("wc-WCDialog");
		if (storeLocatorPopup) {
			closeAllDialogs(); //close other dialogs(quickinfo dialog, etc) before opening this.
			storeLocatorPopup.open();
	...
	function closeStoreLocatorPopup() {
		$('#storeLocatorPopup').WCDialog("close");
	}
	...
	$(document).ready(storeLocatorJS.initProvinceSelections);
</script>
...
<div wcType="RefreshArea" id="selectedStoreListArea" refreshurl="<c:out value='${AjaxSelectedStoreListURL}'/>" declareFunction="selectedStoreListRefreshController()" style="display:block;" ariaMessage="<wcst:message bundle="${widgetText}" key="ACCE_Status_Your_Store_List_Updated"/>" ariaLiveId="${ariaMessageNode}" role="region" aria-labelledby="selectedStoreList_ACCE_Label">
	<% out.flush(); %>
	<c:import url="/Widgets_801/Common/StoreLocatorPopup/SelectedStoreList.jsp">
		...
		<form id='<c:out value="${formName}" />' name='<c:out value="${formName}" />'>
			...
			<div id="WC_StoreLocator_div_32" class="location_select">
				...
				<div id="WC_StoreLocator_div_34" class="location_select_form">
					<div wcType="RefreshArea" id="provinceSelectionsArea" refreshurl="<c:out value="${AjaxProvinceSelectionDisplayURL}"/>" declareFunction="provinceSelectionsRefreshArea()">
						<% out.flush(); %>
						<c:import url="/Widgets_801/Common/StoreLocatorPopup/ProvinceSelectionDisplay.jsp">
							<c:param name="storeId" value="${param.storeId}" />
							<c:param name="catalogId" value="${param.catalogId}" />
							<c:param name="langId" value="${langId}" />
						</c:import>	
			...
			<div id="WC_StoreLocator_div_35" class="location_select">
				...
				<div id="WC_StoreLocator_div_37" class="location_select_form">
					<div wcType="RefreshArea" id="citySelectionsArea" refreshurl="<c:out value="${AjaxCitySelectionDisplayURL}"/>" declareFunction="citySelectionsRefreshController">
						<% out.flush(); %>
						<c:import url="/Widgets_801/Common/StoreLocatorPopup/CitySelectionDisplay.jsp">
							<c:param name="storeId" value="${param.storeId}" />
							<c:param name="catalogId" value="${param.catalogId}" />
							<c:param name="langId" value="${langId}" />
						</c:import>	
...
<div wcType="RefreshArea" id="storeLocatorResultsArea" refreshurl="<c:out value='${AjaxStoreLocatorResultsURL}'/>" ariaMessage="<wcst:message bundle="${widgetText}" key="ACCE_Status_Store_Result_List_Updated"/>" ariaLiveId="${ariaMessageNode}" declareFunction="storeLocatorResultsRefreshController" role="region" aria-labelledby="storeLocatorResults_ACCE_Label">
	<% out.flush(); %>
	<c:import url="/Widgets_801/Common/StoreLocatorPopup/StoreLocatorResults.jsp">
		<c:param name="storeId" value="${param.storeId}" />
		<c:param name="catalogId" value="${param.catalogId}" />
		<c:param name="langId" value="${langId}" />
	</c:import>	
